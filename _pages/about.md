---
permalink: /
title: "👋🏻 Welcome! I'm Kenny"
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---
<div style="display: flex; justify-content: center; align-items: center;">
  <img src="https://scet.berkeley.edu/wp-content/uploads/2ColorPrimary_BE_Formal_Stacked_PRINT_CMYK.png" alt="berkeley eng logo" width="300" style="margin: 20px;"/> 
</div>

I'm a junior at the [**University of California, Berkeley**](https://www.berkeley.edu/), majoring in Industrial Engineering & Operations Research (IEOR) and minoring in Data Science & Mathematics. My research integrates **Operations Research**, **Data Science**, and **Applied Economics** to improve the design and operations of complex systems. I am broadly interested in three research areas:

- **Frontier and Limits of AI in Operations**: I investigate *the reliability of Generative AI* in high-stakes decision systems. My recent work [*(AAAI 2026)*](https://arxiv.org/abs/2511.14214) empirically audits the limits of LLMs in chronological reasoning, identifying systematic failures in how models process time-encoded data to mitigate *lookahead bias* in financial forecasting.

- **Revenue Management and Mechanism Design in Online Platforms**: I apply optimization and machine learning to solve fundamental problems in online marketplaces and urban systems, with a specific focus on pricing for ride-sharing networks. I design *data-driven mechanisms* to prevent incentive misalignments and analyze how algorithmic control impacts market dynamics and stakeholder equity.

- **The Economics of Innovation and The Impact of AI**: I study Human-AI Interaction to understand how emerging technologies reshape organizational behavior and workforce productivity. I advocate for a paradigm of *augmentation* rather than *automation*—where algorithms complement and strengthen human judgment rather than replace it. My work leverages online field experiments, and agentic simulations to explore these evolving dynamics.

This past summer, I was a visiting research fellow at [**Columbia Business School**](https://business.columbia.edu/), collaborating with Professor [**Paul Glasserman**](https://business.columbia.edu/faculty/people/paul-glasserman) of [**Decisions, Risk, and Operations (DRO)**](https://business.columbia.edu/faculty/divisions/dro). At Berkeley, I am fortunate to work with and be advised by Professor [**Chiwei Yan**](https://yanchiwei.github.io/) at the [**Department of Industrial Engineering & Operations Research**](https://ieor.berkeley.edu/), [**Berkeley College of Engineering**](https://engineering.berkeley.edu/), and Professor [**Abishek Nagaraj**](https://haas.berkeley.edu/faculty/nagaraj-abhishek/), at the [**Data Innovation & AI Lab**](https://thedatainnovationlab.com/), [**UC Berkeley Haas School of Business**](https://haas.berkeley.edu/).

A short Curriculum Vitae of mine can be found [here](https://kennywong524.github.io/files/Kenny_CV_2026_updated.pdf).

## 📢 News
- **Nov 2025** — Growing interest in our work on LLM chronological reasoning by AI practitioners on [X](https://x.com/rohanpaul_ai/status/1991344497729388752) (more than 10k views)!
- **Nov 2025** — Our [paper](https://arxiv.org/abs/2511.14214) accepted to the **AI4TS Workshop** (poster) at **AAAI-26**, Singapore and **YURC-26**, New Haven, Connecticut
- **Oct 2025** — Our paper “Do Large Language Models Understand Chronology?” (with Paul Glasserman) accepted for **oral presentation** at the **AAAI-26 Student Abstract Program**, Singapore.
